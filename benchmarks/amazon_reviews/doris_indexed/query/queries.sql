...
-- These queries run against the table WITH inverted indexes
...
-- Q3: Customer history lookup - high-cardinality point query (INVERTED INDEX)
...
-- Q4: Product review analysis - high-cardinality point query (INVERTED INDEX)
...
-- Q5: Review lookup by ID - unique identifier lookup (INVERTED INDEX)
SELECT * FROM amazon_reviews WHERE review_id = 'R1NQ5RXN1LZ0YW';

-- Q6: Range query on ratings - find high-rated reviews (BKD TREE)
-- review_date is epoch days: 16071 = 2014-01-01
...
-- Q7: Multi-dimensional filter - customer + rating range (INVERTED INDEX + BKD TREE)
...
-- Q8: Category + product filter (INVERTED INDEX on product_id)
