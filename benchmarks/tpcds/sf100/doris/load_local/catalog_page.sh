...
echo "[INFO] Stream load ${db}.${table} from: /root/benchmarks/benchmarks/tpcds/sf100/postgresql/data/catalog_page.dat"
...
headers=(
  ...
  -H "columns:cp_catalog_page_sk,cp_catalog_page_id,cp_start_date_sk,cp_end_date_sk,cp_department,cp_catalog_number,cp_catalog_page_number,cp_description,cp_type"
  -H "timeout:${timeout_s}"
  -H "max_filter_ratio:${max_filter_ratio}"
)

if [[ ! -f '/root/benchmarks/benchmarks/tpcds/sf100/postgresql/data/catalog_page.dat' ]]; then
  echo "[ERROR] Source file not found: /root/benchmarks/benchmarks/tpcds/sf100/postgresql/data/catalog_page.dat" >&2
  exit 1
fi
...
file_size_bytes="$(stat -c%s '/root/benchmarks/benchmarks/tpcds/sf100/postgresql/data/catalog_page.dat')"
max_body_bytes="$((max_body_mb * 1024 * 1024))"

if (( file_size_bytes == 0 )); then
  echo "[ERROR] Source file is empty: /root/benchmarks/benchmarks/tpcds/sf100/postgresql/data/catalog_page.dat" >&2
  exit 1
fi
...
if (( file_size_bytes > max_body_bytes )); then
  ...
  split -C "${chunk_mb}m" -d -a 4 '/root/benchmarks/benchmarks/tpcds/sf100/postgresql/data/catalog_page.dat' "$chunk_dir/part_"
...
run_doris_stream_load "$label" '/root/benchmarks/benchmarks/tpcds/sf100/postgresql/data/catalog_page.dat' "full file"
