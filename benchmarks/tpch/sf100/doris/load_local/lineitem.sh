...
echo "[INFO] Stream load ${db}.${table} from: /root/benchmarks/benchmarks/tpch/sf100/postgresql/data/lineitem.tbl"
...
headers=(
  ...
  -H "columns:l_orderkey,l_partkey,l_suppkey,l_linenumber,l_quantity,l_extendedprice,l_discount,l_tax,l_returnflag,l_linestatus,l_shipdate,l_commitdate,l_receiptdate,l_shipinstruct,l_shipmode,l_comment"
  -H "timeout:${timeout_s}"
  -H "max_filter_ratio:${max_filter_ratio}"
)

if [[ ! -f '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/lineitem.tbl' ]]; then
  echo "[ERROR] Source file not found: /root/benchmarks/benchmarks/tpch/sf100/postgresql/data/lineitem.tbl" >&2
  exit 1
fi
...
file_size_bytes="$(stat -c%s '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/lineitem.tbl')"
max_body_bytes="$((max_body_mb * 1024 * 1024))"

if (( file_size_bytes == 0 )); then
  echo "[ERROR] Source file is empty: /root/benchmarks/benchmarks/tpch/sf100/postgresql/data/lineitem.tbl" >&2
  exit 1
fi
...
if (( file_size_bytes > max_body_bytes )); then
  ...
  split -C "${chunk_mb}m" -d -a 4 '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/lineitem.tbl' "$chunk_dir/part_"
...
run_doris_stream_load "$label" '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/lineitem.tbl' "full file"
