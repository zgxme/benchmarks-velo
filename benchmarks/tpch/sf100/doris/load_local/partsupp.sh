...
echo "[INFO] Stream load ${db}.${table} from: /root/benchmarks/benchmarks/tpch/sf100/postgresql/data/partsupp.tbl"
...
headers=(
  ...
  -H "columns:ps_partkey,ps_suppkey,ps_availqty,ps_supplycost,ps_comment"
  -H "timeout:${timeout_s}"
  -H "max_filter_ratio:${max_filter_ratio}"
)

if [[ ! -f '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/partsupp.tbl' ]]; then
  echo "[ERROR] Source file not found: /root/benchmarks/benchmarks/tpch/sf100/postgresql/data/partsupp.tbl" >&2
  exit 1
fi
...
file_size_bytes="$(stat -c%s '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/partsupp.tbl')"
max_body_bytes="$((max_body_mb * 1024 * 1024))"

if (( file_size_bytes == 0 )); then
  echo "[ERROR] Source file is empty: /root/benchmarks/benchmarks/tpch/sf100/postgresql/data/partsupp.tbl" >&2
  exit 1
fi
...
if (( file_size_bytes > max_body_bytes )); then
  ...
  split -C "${chunk_mb}m" -d -a 4 '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/partsupp.tbl' "$chunk_dir/part_"
...
run_doris_stream_load "$label" '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/partsupp.tbl' "full file"
