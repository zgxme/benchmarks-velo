...
echo "[INFO] Stream load ${db}.${table} from: /root/benchmarks/benchmarks/tpch/sf100/postgresql/data/region.tbl"
...
headers=(
  ...
  -H "columns:r_regionkey,r_name,r_comment"
  -H "timeout:${timeout_s}"
  -H "max_filter_ratio:${max_filter_ratio}"
)

if [[ ! -f '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/region.tbl' ]]; then
  echo "[ERROR] Source file not found: /root/benchmarks/benchmarks/tpch/sf100/postgresql/data/region.tbl" >&2
  exit 1
fi
...
file_size_bytes="$(stat -c%s '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/region.tbl')"
max_body_bytes="$((max_body_mb * 1024 * 1024))"

if (( file_size_bytes == 0 )); then
  echo "[ERROR] Source file is empty: /root/benchmarks/benchmarks/tpch/sf100/postgresql/data/region.tbl" >&2
  exit 1
fi
...
if (( file_size_bytes > max_body_bytes )); then
  ...
  split -C "${chunk_mb}m" -d -a 4 '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/region.tbl' "$chunk_dir/part_"
...
run_doris_stream_load "$label" '/root/benchmarks/benchmarks/tpch/sf100/postgresql/data/region.tbl' "full file"
