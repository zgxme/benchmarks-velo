...
echo "[INFO] Stream load ${db}.${table} from: /root/benchmarks/benchmarks/ssb/sf100/postgresql/data/lineorder.tbl"
...
headers=(
  ...
  -H "columns:lo_orderkey,lo_linenumber,lo_custkey,lo_partkey,lo_suppkey,lo_orderdate,lo_orderpriority,lo_shippriority,lo_quantity,lo_extendedprice,lo_ordtotalprice,lo_discount,lo_revenue,lo_supplycost,lo_tax,lo_commitdate,lo_shipmode"
  -H "timeout:${timeout_s}"
  -H "max_filter_ratio:${max_filter_ratio}"
)

if [[ ! -f '/root/benchmarks/benchmarks/ssb/sf100/postgresql/data/lineorder.tbl' ]]; then
  echo "[ERROR] Source file not found: /root/benchmarks/benchmarks/ssb/sf100/postgresql/data/lineorder.tbl" >&2
  exit 1
fi
...
file_size_bytes="$(stat -c%s '/root/benchmarks/benchmarks/ssb/sf100/postgresql/data/lineorder.tbl')"
max_body_bytes="$((max_body_mb * 1024 * 1024))"

if (( file_size_bytes == 0 )); then
  echo "[ERROR] Source file is empty: /root/benchmarks/benchmarks/ssb/sf100/postgresql/data/lineorder.tbl" >&2
  exit 1
fi
...
if (( file_size_bytes > max_body_bytes )); then
  ...
  split -C "${chunk_mb}m" -d -a 4 '/root/benchmarks/benchmarks/ssb/sf100/postgresql/data/lineorder.tbl' "$chunk_dir/part_"
...
run_doris_stream_load "$label" '/root/benchmarks/benchmarks/ssb/sf100/postgresql/data/lineorder.tbl' "full file"
