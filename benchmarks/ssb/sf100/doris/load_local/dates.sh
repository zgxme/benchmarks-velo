...
echo "[INFO] Stream load ${db}.${table} from: /root/benchmarks/benchmarks/ssb/sf100/postgresql/data/dates.tbl"
...
headers=(
  ...
  -H "columns:d_datekey,d_date,d_dayofweek,d_month,d_year,d_yearmonthnum,d_yearmonth,d_daynuminweek,d_daynuminmonth,d_daynuminyear,d_monthnuminyear,d_weeknuminyear,d_sellingseason,d_lastdayinweekfl,d_lastdayinmonthfl,d_holidayfl,d_weekdayfl"
  -H "timeout:${timeout_s}"
  -H "max_filter_ratio:${max_filter_ratio}"
)

if [[ ! -f '/root/benchmarks/benchmarks/ssb/sf100/postgresql/data/dates.tbl' ]]; then
  echo "[ERROR] Source file not found: /root/benchmarks/benchmarks/ssb/sf100/postgresql/data/dates.tbl" >&2
  exit 1
fi
...
file_size_bytes="$(stat -c%s '/root/benchmarks/benchmarks/ssb/sf100/postgresql/data/dates.tbl')"
max_body_bytes="$((max_body_mb * 1024 * 1024))"

if (( file_size_bytes == 0 )); then
  echo "[ERROR] Source file is empty: /root/benchmarks/benchmarks/ssb/sf100/postgresql/data/dates.tbl" >&2
  exit 1
fi
...
if (( file_size_bytes > max_body_bytes )); then
  ...
  split -C "${chunk_mb}m" -d -a 4 '/root/benchmarks/benchmarks/ssb/sf100/postgresql/data/dates.tbl' "$chunk_dir/part_"
...
run_doris_stream_load "$label" '/root/benchmarks/benchmarks/ssb/sf100/postgresql/data/dates.tbl' "full file"
